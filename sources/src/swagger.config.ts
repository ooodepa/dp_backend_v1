...
const config = new DocumentBuilder()
  ...
  .addTag('user', 'Для зарег. пользователей (нужен токен доступа)')
  .addTag('ADMIN', 'Для зарег. пользователей (нужен access токен и роль ADMIN)')
  .addTag('MODER', 'Для зарег. пользователей (нужен access токен и роль MODER)')
  .addBearerAuth(
    {
      type: 'http',
      scheme: 'bearer',
